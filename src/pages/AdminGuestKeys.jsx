  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Reschedule Guest Key</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">Default: 30 min before trip start</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <input type="number" min={0} max={1440} value={revokeOffset}
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <p className="text-xs text-gray-400 mt-1">Default: 60 min after trip end</p>
          </div>
...
function EditTimesModal({ booking, onClose, onSuccess, api }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Edit Access Times</h3>
        <p className="text-sm text-gray-500 mb-1">
          Ad-hoc key · <span className="font-mono">{booking.bookingId}</span>
        </p>
        {booking.label && (
          <p className="text-xs text-gray-400 mb-4">{booking.label}</p>
        )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Create Ad-hoc Guest Key</h3>
        <p className="text-sm text-gray-500 mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Label */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Purpose / Label</label>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" min={0} max={1440} value={activateOffset}
                  onChange={e => setActivateOffset(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <p className="text-xs text-gray-400 mt-1">0 = send immediately on create</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input type="number" min={0} max={1440} value={revokeOffset}
                  onChange={e => setRevokeOffset(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <p className="text-xs text-gray-400 mt-1">0 = revoke exactly at end time</p>
              </div>
            </div>
          </details>
    ...
    <AdminLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Tesla Guest Keys</h1>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={fetchKeys}
              className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50 shadow-sm"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {filtered.map(k => {
                    ...
                    return (
                      <tr key={k.bookingId} className="hover:bg-gray-50 transition-colors">
                        {/* Booking */}
                        ...
                        <td className="px-4 py-3">
                          ...
                          {guestEmail && (
                            <div className="text-xs text-gray-400 truncate max-w-[160px]">{guestEmail}</div>
                          )}
                        </td>

                        {/* Vehicle */}
                        <td className="px-4 py-3">
                          <div className="text-sm text-gray-900">{k.vehicleName || k.vin || "—"}</div>
                          {k.vin && <div className="text-xs text-gray-400 font-mono">{k.vin}</div>}
                        </td>
